...
module.exports = async (req, res) => {
  ...
  try {
    ...
    while (body[`leads[status][${i}][id]`] !== undefined) {
      ...
      if (statusId === QUALIFIED_ID) {
        const info = await fetchLeadInfo(leadId);
        if (info) await sendQualifiedLead(leadId, info);
      }
      i++;
    }

    // To'g'ridan-to'g'ri etapda yaratilgan lead
    let j = 0;
...
// ── AmoCRM dan kontakt ma'lumotlarini olish ───────────────────────────────
async function fetchLeadInfo(leadId) {
  ...
  try {
    ...
    for (const f of contact.custom_fields_values || []) {
      ...
      if (f.field_code === 'EMAIL' && f.values?.[0]?.value) email = f.values[0].value.trim().toLowerCase();
      if (FIELD_FBP && f.field_id === FIELD_FBP) fbp = f.values?.[0]?.value || '';
      if (FIELD_FBC && f.field_id === FIELD_FBC) fbc = f.values?.[0]?.value || '';
    }

    return { phone, firstName, email, fbp, fbc, price: lead.price || 0 };

  } catch (err) {
    console.error('[AMO] Lead olishda xatolik:', err);
    return null;
  }
}
...
async function sendPurchase(leadId, data) {
  if (!PIXEL_ID || !ACCESS_TOKEN) { console.warn('[META PURCHASE] Credentials yo\'q'); return; }
  ...
  const payload = {
    data: [{
      ...
      user_data:        userData,
      custom_data: { value: Math.round(data.price / 12000), currency: 'USD' },
    }],
  };
...
// ── QualifiedLead ─────────────────────────────────────────────────────────
async function sendQualifiedLead(leadId, data) {
  if (!PIXEL_ID || !ACCESS_TOKEN) { console.warn('[META QUALIFIED] Credentials yo\'q'); return; }
  ...
  const result = await res.json();
  if (!res.ok) console.error('[META QUALIFIED ERROR]', result);
  else         console.log('[META QUALIFIED] QualifiedLead yuborildi, lead:', leadId);
  return result;
}
